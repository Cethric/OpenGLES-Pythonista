# coding: utf-8
"""
NOTE THIS IS DEPRECIATED AND NO LONGER USED. I HAVE ONLY KEPT IT FOR A POINT
OF REFERENCE. DO NOT USE

Attributes:
    PHYSICS_DIR (str): The location of this script
    BULLET_CODE (str): the location of ammo.js
    CHART_CODE (str): the location of Chart.min.js
    BULLET_HTML (str): a html string which can be used for ammo.js data
    PHYSICS_WORLD_LOAD (str): A javascript string to load the physics world
    ADD_SPHERE, ADD_BOX, ADD_SHAPE: Javascript strings to add 'shapes' to the physics world
    GET_POSITION: Javascript function to get the position of an object
    GET_ROTATION: Javascript function to get the rotation of an object
    PhysicsWorld: A physics world object (OpenGLES.Util.Physics.Bullet)
"""
import objc_util
...
if os.path.exists(CHART_CODE):
    with open(CHART_CODE, "rb") as f:
        CHART = f.read()
else:
...
class Bullet(object):
    def __init__(self):
        """
        Container class to handle most physics related functions
        Attributes:
            setup (bool): Have the libraries been loaded
            wv (ui.WebView): the webview object to contain the JSContext
            frame (tuple(int)): a frame tuple
            ids (list(int)): the ids of all physics objects
            objdata (dict): a dict containing the location and rotation of an object
            can_check (bool): depreciated value for use with polling variables
        """
        self.setup = False
        self.wv = ui.WebView()
    ...
    def webview_should_start_load(self, webview, url, nav_type):
        """
        Check if a url should be loaded
        Args:
            webview (ui.WebView): the web view that wants to load
            url (str): the url to load
            nav_type (str): the type of navigation
        Returns:
            (bool): True if 'python' not in url
        """
        r = not 'python' in url
        if not r:
    ...
    def ios_log(self, args):
        """
        Simple method to log JS events
        Args:
            args (str): the message to print
        """
        print "BULLET-LOG:", args
        
    def ios_print(self, args):
        """
        Simple method to print js data
        Args:
            args (str): the message to print
        """
        print "BULLET:", args
    ...
    def ios_error(self, args):
        """
        Simple method to log JS errors
        Args:
            args (str): the error message to print
        """
    ...
    def ios_alert(self, args):
        """
        Create a console.alert for js events
        Args:
            args (str): the js alert to show
        """
        dialogs.alert('BULLET-ALERT', args)
    
    def _get_position(self, args):
        """
        Save the postion of the object to a python attribute
        Args:
            args (str): the js data
        """
        args = args.split("#")
    ...
    def _get_rotation(self, args):
        """
        Save the rotation of the object to a python attribute
        Args:
            args (str): the js data
        """
        args = args.split("#")
    ...
    def webview_did_finish_load(self, webview):
        """
        If the webview finished loading.
        If the Physics world and its libraries have not been setup do so.
        Args:
            webview (ui.WebView): the webview that finished loading
        """
        if not self.setup:
            self.wv.evaluate_javascript(AMMO)
            self.wv.evaluate_javascript(PHYSICS_WORLD_LOAD)
            self.wv.evaluate_javascript(CHART)
            self.wv.evaluate_javascript("var ctx = document.getElementById('stepchart').getContext('2d');var sc = new Chart(ctx).Line({});")
            self.setup = True
        
    def webview_did_fail_load(self, webview, error_code, error_msg):
        """
        Simple handler for webview loading errors
        Args:
            webview (ui.WebView): the webview that failed to load
            error_code (str): the error code raised
            error_msg (str): the error message raised
        """
    ...
    def add_object(self, vert, mass=1, pos=[0,0,0], convex=False):
        ...
        Args:
            vert (list[float]): a vertex array
            mass (Optional[float]): the mass of the object
            pos (Optional[float]): a list containig the initial x,y,z
                                   position of the object
            convex (bool): Use ConvexHullShape instead of a TriangleMesh
        Returns:
    ...
    def add_sphere(self, mass=1, pos=[0,0,0], radius=1):
        ...
        Args:
            mass (Optional[float]): the mass of the object
            pos (Optional[list[float]]): the initial x,y,z position of the object
    ...
    def add_box(self, mass=1, pos=[0,0,0], size=1):
        ...
        Args:
            mass (Optional[float]): the mass of the object
            pos (Optional[list[float]]): the initial x,y,z position of the object
            size (Optional[float]): the size of the box
        Returns:
            (int): the id of the object
        """
    ...
    def step_simulation(self, step, maxSubStep=1, fixTimeStep=1/60.0):
        """
        Step the simulation on frame
        Args:
            step (float): the size of the step
            maxSubStep (Optional[float]): the maximum size of sub steps
            fixTimeStep (Optional[float]): the fixed step size that a substep can take
        """
        start = time.clock()
    ...
    def get_object_pos(self, i=0):
        ...
        Args:
            i (int): the id of the object
        Returns:
            (euclid.Vector3): the position of the object
        """
        start = time.clock()
        i = int(i)
        vpos = euclid.Vector3()
    ...
    def _get_object_pos(self, i):
        """
        Private method to get object position
        Args:
            i (int): the id of the object
        """
    ...
    def get_object_rot(self, i=0):
        ...
        Args:
            i (int): the id of the object
        Returns:
            (euclid.Quaternion): the rotation of the object
        """
        start = time.clock()
        i = int(i)
    ...
    def _get_object_rot(self, i):
        """
        Private method to get object rotation
        Args:
            i (int): the id of the object
        """
